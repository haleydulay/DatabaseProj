...
vector<string> Scmax;
double storage = 0;
vector<vector<string> > C(DBLP100K);
vector<vector<string> > Sc(DBLP100K);
vector<vector<string> > D(DBLP100K);
vector<vector<string> > Dc(DBLP100K);
...
cout << endl << "Would you like to view the dynamic phase's clustering result? 0 for No and 1 for Yes: ";
cin >> choice;
...
double time_taken = double(end-beg) / double(CLOCKS_PER_SEC);
cout << endl << "Scalability experiments had an execution time of about: "<< time_taken << " seconds" << endl;
