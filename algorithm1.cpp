...
double record_similarity(vector<string> c, vector<string> d)
{
double result = 0;
...
double cluster_similarity(vector<string> r, vector<string> r2)
{
double result = 0;
result += record_similarity(r,r2);
...
if(experiment==t1)
{
    ...
    for(int i=0;i<R.size();i++) // for all r in R do    //can change the size to DBLP-WW possibly if not exact # of records
    {
        for(int j=0;j<C.size();j++)  // for all C in C do
            storage += cluster_similarity(R[i],Sc[j]);
	
	if(storage!=0)
		cout << storage / Sc.size();
	if(i==1)
		break;
...
/*
        ...
        {      
            ...
            out.C[i].push_back(Cmax);                       // C <- C union Cmax
            out.Sc[i].push_back(Scmax);                       // Sc <- Sc union sCmax
        }   
    }
